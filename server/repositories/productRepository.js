...
export async function updateProduct(id, p) {
  const db = await getDB()
  await db.run(`UPDATE products SET codigo = ?, nombre = ?, descripcion = ?, precio_venta = ?, costo_unitario = ?, costo_promedio = ?, stock_actual = ?, stock_minimo = ?, estado = ?, id_categoria = ?, id_proveedor = ? WHERE id = ?`,
    p.codigo, p.nombre, p.descripcion, p.precio_venta, p.costo_unitario, p.costo_promedio, p.stock_actual, p.stock_minimo, p.estado, p.id_categoria, p.id_proveedor, id)
  return getProductById(id)
}
